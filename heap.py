...
class Heap:
    ...
    def bubbleup(self, idx):
        ...
        while self.h[pos] < self.parent(pos):
            # swap with parent
            self.h[pos], self.h[self.parentpos(
                pos)] = self.h[self.parentpos(pos)], self.h[pos]
    ...
    def bubbledown(self, idx):
        ...
        while min(self.left(pos), self.right(pos)) > self.h[pos]:
            if self.left(pos) < self.right(pos):
                ...
                self.h[pos], self.h[self.leftpos(
                    pos)] = self.h[self.leftpos(pos)], self.h[pos]
                pos = self.leftpos(pos)
            else:
                # swap with right child
                self.h[pos], self.h[self.rightpos(
                    pos)] = self.h[self.rightpos(pos)], self.h[pos]
